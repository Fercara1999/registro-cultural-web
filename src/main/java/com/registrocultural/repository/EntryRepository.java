...
public interface EntryRepository extends JpaRepository<Entry, Integer> {
    ...
    @Query("SELECT e FROM Entry e WHERE e.type LIKE '%Libro%' AND e.title = :title AND e.author IS NOT NULL ORDER BY e.id DESC")
    List<Entry> findAuthorForTitle(@Param("title") String title);

    @Query("SELECT e FROM Entry e WHERE e.type LIKE '%Pel%' AND e.title = :title AND e.director IS NOT NULL ORDER BY e.id DESC")
    List<Entry> findDirectorForTitle(@Param("title") String title);

    @Query("SELECT e FROM Entry e WHERE e.type LIKE '%Serie%' AND e.title = :title ORDER BY e.season DESC, e.episode DESC")
